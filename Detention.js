...
app
    ...
    .get('/getProfiles', (req,res)=>{
        const sql = `SELECT Name, Status, classAllocation FROM login_info WHERE Status != 'TBD'` 
        const sql2 = `SELECT Name, Duty, Week FROM duties`
    ...
    .post('/allocations', (req,res)=>{
        if(req.body.subject == 'classA'){
            const sql = 'UPDATE login_info SET classAllocation = ? WHERE Name = ?'
            connection.query(sql, [req.body.text, req.body.name], (err, results)=>{
                if(err){
                    console.log(err)
                }
            })
            ...
            const sql = 'INSERT INTO duties (Name, Duty) VALUES (?, ?)'
            connection.query(sql, [req.body.name, req.body.text], (err, results)=>{
                if(err){
                    console.log(err)
                }
            })
